<template>
  ...
  <div class="flex justify-around">
    <Iconify
      :icon="item.icon"
      class="cursor-pointer text-2xl text-gray-200 hover:text-sky-500"
...
const rules = reactive<FormRules<LoginFormInterface>>({
  ...
  remeber: [{ type: 'boolean', required: true, message: '请记住账号', trigger: 'change' }]
})

const onSubmit = (form: any) => {
  if (formRef.value) {
    formRef.value.validate((valid: any) => {
      if (valid) emits('submit', form)
    })
  }
}
